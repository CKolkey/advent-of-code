...
PROGRAM = [
  1, 12, 2, 3, 1, 1, 2, 3, 1, 3, 4, 3, 1, 5, 0, 3, 2, 13, 1, 19, 1, 5, 19, 23,
...
loop do
  instruction, value_a_address, value_b_address, address = PROGRAM[pointer..(pointer + 3)]

  value_a = PROGRAM[value_a_address]
  value_b = PROGRAM[value_b_address]

  case instruction
  when 1
    PROGRAM[address] = value_a + value_b
  when 2
    PROGRAM[address] = value_a * value_b
  when 99
    break
