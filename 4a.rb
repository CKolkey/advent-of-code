...
def valid_keys?(passport)
  return false unless (1920..2002).cover? passport["byr"].to_i
...
def valid_height?(height)
  ...
  range.cover? height.to_i
end

def count_valid_passports
  split_passports.count do |pass|
    (pass.keys.size == 8 || (pass.keys.size == 7 && pass["cid"].nil?)) && valid_keys?(pass)
  end
end

puts count_valid_passports
